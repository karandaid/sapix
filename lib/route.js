...
class SapixRoutes {
    ...
    async routeHandle(req, res){
        ...
        if (this.routes[method]) {
            ...
            let doesItEverMentioned = false;

            if(!routePath) {
                routePath = matchURLToPatterns([path], Object.keys(this.routes[method])).get(path)
                doesItEverMentioned = this.routes[method].hasOwnProperty(`/${routePath.required_key}`)
            ...
            let routeFunctionHandler = cache.get(`handler_${routePath.pattern}_${method}`)
            let routePathParameters = routePath.params

            if(!routeFunctionHandler){
                // Check if the handler exists in the routes object
                routeFunctionHandler = this.routes[method][`/${routePath.required_key}`]
                ...
                if (routeFunctionHandler) {
                    cache.set(`handler_${routePath.pattern}_${method}`, routeFunctionHandler);
                }
            }
